...
ssh "$DEPLOY_USER"@"$DEPLOY_HOST" -o StrictHostKeyChecking=no 'rm -rf tmp/old.build && mv web/dashingdog-blog-admin/dist tmp/old.build && cp -r tmp/$TRAVIS_BUILD_ID/ web/dashingdog-blog-admin/dist'
echo 'deploy success'
